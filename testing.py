...
def train(args, model, device, train_loader_lr, train_loader_hr, optimizer, epoch):
	...
	for (batch_idx, (data, t)), (target, p) in zip(enumerate(train_loader_lr), train_loader_hr):
		...
		if batch_idx == (len(train_loader_lr)-1):
...
def main():
	...
	test_loader_lr = torch.utils.data.DataLoader(data, batch_size=args.batch_size, sampler=test_sampler_lr)
	test_loader_hr = torch.utils.data.DataLoader(target, batch_size=args.batch_size, sampler=test_sampler_hr)
